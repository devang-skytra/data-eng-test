...
qfile="index.X7_DOI_no_spot.sql"
bfile="index.X7_DOI_no_spotbash.sql"

cd ~/indexproduction/af/dags/sq
cp $qfile $bfile

sed -i  -e "s/{{ ti.xcom_pull('Get-TsX7_DOI_NS')[0] }} AS pl_id/0 AS pl_id/g" \
$bfile

bq query --location=EU --replace --allow_large_results --use_legacy_sql=false --destination_table    "$(cat $bfile)"
...
uk="CONCAT( ifnull(region_pair,'null_rp'), '|', ifnull(dt_of_issue,'2000-01-01') )"
fp="IFNULL ( CONCAT( round(iata_index,4),'|',round(skytra_index,4),'|',round(iata_rpk,1),'|',round(skytra_rpk,1),'|',iata_tickets,'|',skytra_tickets,'|',round(skytra_coverage,4) ), 'null_attr')"
