...
qfile="X7_researchlicense_4_4_0.sql"
bfile="X7_researchlicense_4_4_0bash.sql"

cd ~/indexproduction/af/dags/sq
cp $qfile $bfile

sed -i  -e "s/{{ ti.xcom_pull('Get-TsX7rl')[0] }} AS pl_id/0 AS pl_id/g" \
$bfile

# APPEND!
bq query --location=EU --append_table --allow_large_results --use_legacy_sql=false --destination_table $ds_tb "$(cat $bfile)"
...
uk="CONCAT( ifnull(region_pair,'null_rp'), '|', ifnull(date,'2000-01-01') )"
fp="IFNULL ( CONCAT( round(skytra_index,4),'|',round(skytra_benchmark,4),'|',round(skytra_issued_index,4),'|',round(skytra_issued_benchmark,4) ), 'null_attr')"
wh="where date between '2020-05-01' and '2020-05-07'"
...
/*
select 'in113B_X7_RECON' as tbl, * from scratch_PaD_EU.in113B_X7_RECON where match = false or match is null  UNION ALL
select 'in113B_X7_IATA_RECON' as tbl, * from scratch_PaD_EU.in113B_X7_IATA_RECON where match = false or match is null  UNION ALL
select 'in113B_X7_DOI_no_spot_RECON' as tbl, * from scratch_PaD_EU.in113B_X7_DOI_no_spot_RECON where match = false or match is null  UNION ALL
select 'in113B_X7_RL_4_4_RECON' as tbl, * from scratch_PaD_EU.in113B_X7_RL_4_4_RECON where match = false or match is null
*/
