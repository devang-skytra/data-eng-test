...
sed -i  -e "s/{{ ti.xcom_pull('Get-TsX7')[0] }} AS pl_id/0 AS pl_id/g" \
$bfile
...
fp="IFNULL ( CONCAT( iata_index,'|',skytra_index,'|',ROUND(iata_rpk,1),'|',ROUND(skytra_rpk,1),'|',iata_tickets,'|',skytra_tickets,'|',skytra_coverage), 'null_attr')"
...
ifnull(x.fp,'null_fp') = ifnull(xin.fp,'null_fp') as match
