...
CREATE OR REPLACE PROCEDURE `skytra-benchmark-rnd.iata_sp.sp_process_X3_daily`(IN from_date_int INT64, IN to_date_int INT64)
/*modf PD 20200727 CREATE OR REPLACE PROCEDURE `skytra-benchmark-rnd.iata_sp.sp_process_X3`(IN from_date DATE, IN to_date DATE, IN gcsFileFilter STRING)*/
BEGIN
  --CALL `skytra-benchmark-rnd.iata_sp.sp_process_X3_daily`(20200720,20200726);
  ...
  /*added PD 20200727*/
  DECLARE from_file STRING;
  DECLARE to_file STRING;
  SET from_file = CONCAT('IATA_daily_api_', CAST(from_date_int AS STRING));
  SET to_file = CONCAT('IATA_daily_api_', CAST(to_date_int AS STRING));
  /**/
  
  SET bq_ts = @@script.creation_time;
  CALL log_sp.sp_proc_load_start(bq_ts, dset, dtbl, DATE'2020-07-20', DATE'2020-07-26', uid);
...
----------------------------------------------------------------------------------------------------
  ...
  iata_input AS(
    SELECT * EXCEPT(DT_OF_ISSUE, FLIGHT_DATE, FLIGHT_DEPART_TIME, FLIGHT_ARRIVAL_DATE,FLIGHT_ARRIVAL_TIME,file_date_int),
    -- Reformat date to comply with historical CSV, remove '-''
    REGEXP_REPLACE(DT_OF_ISSUE,"-","") AS DT_OF_ISSUE,
    REGEXP_REPLACE(FLIGHT_DATE,"-","") AS FLIGHT_DATE,
    REGEXP_REPLACE(FLIGHT_ARRIVAL_DATE,"-","") AS FLIGHT_ARRIVAL_DATE,
    -- Reformat times to comply with historical CSV, take the first 5 characters remove ':'
    REGEXP_REPLACE(SUBSTR(FLIGHT_DEPART_TIME,0,5),":","") AS FLIGHT_DEPART_TIME,
    REGEXP_REPLACE(SUBSTR(FLIGHT_ARRIVAL_TIME,0,5),":","") AS FLIGHT_ARRIVAL_TIME,
    CONCAT('IATA_daily_api_', CAST(file_date_int AS STRING)) AS FILE_SOURCE,
    SAFE_CAST(segment_id AS INT64) AS seg_id
    /*modf PD 20200727
    FROM iata.int_X1_file_analysis
    WHERE file_source LIKE CONCAT('%',gcsFileFilter,'%')
    */
    FROM iata.X1
    WHERE file_date_int between from_date_int and to_date_int
  );
  ...
  all_iata_casted AS (
    ...
    SELECT
      ...
      WHEN TRIM(flight_depart_time) = '' THEN TRUE
      WHEN TRIM(flight_arrival_time) = '' THEN TRUE
...
----------------------------------------------------------------------------------------------------
  ...
  CALL log_sp.sp_proc_load_end(DATE'2020-07-20', uid, jobInfo);
